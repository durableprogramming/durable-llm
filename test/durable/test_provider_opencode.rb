...
class TestProviderOpencode < Minitest::Test
  def setup
    WebMock.disable_net_connect!
    @provider = Durable::Llm::Providers::Opencode.new(api_key: 'test_api_key')
  end
  ...
  def test_default_api_key
    ENV['OPENCODE_API_KEY'] = 'env_api_key'
    provider = Durable::Llm::Providers::Opencode.new
    assert_equal 'env_api_key', provider.default_api_key
    ENV.delete('OPENCODE_API_KEY')
  end

  def test_completion
    stub_request(:post, 'https://opencode.ai/zen/v1/chat/completions')
    ...
    assert_instance_of Durable::Llm::Providers::Opencode::OpencodeResponse, response
    assert_equal 'Test response', response.choices.first.to_s
  end

  def test_embedding
    stub_request(:post, 'https://opencode.ai/zen/v1/embeddings')
      .to_return(status: 200, body: {
        data: [
    ...
    assert_instance_of Durable::Llm::Providers::Opencode::OpencodeEmbeddingResponse, response
    assert_equal [0.1, 0.2, 0.3], response.embedding.map(&:to_f)
  end

  def test_models
    stub_request(:get, 'https://opencode.ai/zen/v1/models')
      .to_return(status: 200, body: {
        data: [
          { id: 'gpt-3.5-turbo' },
          { id: 'gpt-4' }
